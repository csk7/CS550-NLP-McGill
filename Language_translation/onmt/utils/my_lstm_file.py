...
class bin_weights(torch.autograd.Function):
    @staticmethod
    def forward(ctx, x):
        return binarize(x)
    @staticmethod
...
class RNNBase(nn.modules.RNNBase):

    def __init__(self, mode, input_size, hidden_size,
                 num_layers=1, bias=True, batch_first=False,
                 dropout=0, bidirectional=False):
        super(RNNBase, self).__init__(mode,input_size,hidden_size,num_layers=1, bias=True, batch_first=False,dropout=0, bidirectional=False)
        self.mode = mode
    ...
    def forward(self, input, hx=None):
        ...
        with torch.no_grad():
            self.Wb=[]
            for i in range(len(self.all_weights[0])):
                self.Wb.append(quant_weights.apply(self.all_weights[0][i]))
            self.Wb_final = []
            self.Wb_final.append(self.Wb)


            #input_b = quant_weights.apply(input)
        

        output, hidden = func(input, self.Wb_final, hx, batch_sizes)
...
class LSTMCell(nn.modules.LSTMCell):
    def __init__(self, input_size, hidden_size, bias=True):
        super(LSTMCell, self).__init__(input_size, hidden_size, bias=True)
        self.input_size = input_size
        self.hidden_size = hidden_size
        self.bias = bias
    ...
    def forward(self, input, hx=None):
        ...
        with torch.no_grad():
            self.weight_ih_b = quant_weights.apply(self.weight_ih)
            self.weight_hh_b = quant_weights.apply(self.weight_hh)
            self.bias_ih_b = quant_weights.apply(self.bias_ih)
            self.bias_hh_b = quant_weights.apply(self.bias_hh)

        #return self._backend.LSTMCell(input, hx,self.weight_ih, self.weight_hh,self.bias_ih, self.bias_hh)
        return self._backend.LSTMCell(input, hx,self.weight_ih_b, self.weight_hh_b,self.bias_ih_b, self.bias_hh_b)
